...
function ProfileAppearanceForm({ session, onSaved }: { session: Session; onSaved: () => void | Promise<void> }) {
  const { authenticate } = useSession();
  const [form, setForm] = useState({ avatarKey: session.actor.avatarKey, accentKey: session.actor.accentKey });
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState<{ tone: 'success' | 'error'; text: string } | null>(null);
  const save = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setMessage(null);
    try {
      const result = await postJson<{ session: Session }>('/parent/profile', form, 'PUT');
      authenticate(result.session);
      await onSaved();
      setMessage({ tone: 'success', text: 'Your colour theme is now active.' });
    } catch (caught) { setMessage({ tone: 'error', text: caught instanceof Error ? caught.message : 'Your profile appearance could not be saved.' }); }
    finally { setBusy(false); }
  };
  return <form className="profile-appearance-form" data-theme={form.accentKey} onSubmit={save}><div className="profile-appearance-details"><Avatar avatarKey={form.avatarKey} accentKey={form.accentKey} size="lg" label="Your avatar preview" /><Field label="Avatar"><Select value={form.avatarKey} onChange={(event) => setForm({ ...form, avatarKey: event.target.value })}>{avatarOptions.filter((option) => option.key.startsWith('grownup')).map((option) => <option key={option.key} value={option.key}>{option.label}</option>)}</Select></Field></div><ColourPicker value={form.accentKey} onChange={(accentKey) => setForm({ ...form, accentKey })} label="Colour theme" />{message ? <InlineNotice tone={message.tone}>{message.text}</InlineNotice> : null}<div className="settings-save"><Button type="submit" disabled={busy}>{busy ? 'Saving…' : 'Save my appearance'}</Button></div></form>;
}

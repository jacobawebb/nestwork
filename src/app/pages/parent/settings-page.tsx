...
function ProfileAppearanceForm({ session, onSaved }: { session: Session; onSaved: () => void | Promise<void> }) {
  ...
  return <form className="profile-appearance-form" data-theme={form.accentKey} onSubmit={save}><Avatar avatarKey={form.avatarKey} accentKey={form.accentKey} size="lg" label="Your avatar preview" /><div className="form-stack"><Field label="Avatar"><Select value={form.avatarKey} onChange={(event) => setForm({ ...form, avatarKey: event.target.value })}>{avatarOptions.filter((option) => option.key.startsWith('grownup')).map((option) => <option key={option.key} value={option.key}>{option.label}</option>)}</Select></Field><ColourPicker value={form.accentKey} onChange={(accentKey) => setForm({ ...form, accentKey })} label="Colour theme" />{message ? <InlineNotice tone={message.tone}>{message.text}</InlineNotice> : null}<div className="settings-save"><Button type="submit" disabled={busy}>{busy ? 'Saving…' : 'Save my appearance'}</Button></div></div></form>;
}

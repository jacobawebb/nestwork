...
function ChildEditor({ child, onClose, onSaved }: { child?: ChildPerson | null; onClose: () => void; onSaved: () => void | Promise<void> }) {
  ...
  return <form className="form-stack" onSubmit={save}><div className="avatar-preview"><Avatar avatarKey={avatarKey} accentKey={accentKey} size="lg" /></div><Field label="Display name"><TextInput value={displayName} onChange={(event) => setDisplayName(event.target.value)} maxLength={50} required autoFocus /></Field><div className="form-grid"><Field label="Avatar"><Select value={avatarKey} onChange={(event) => setAvatarKey(event.target.value)}>{avatarOptions.filter((option) => option.key.startsWith('child')).map((option) => <option key={option.key} value={option.key}>{option.label}</option>)}</Select></Field><Field label="Accent colour"><Select value={accentKey} onChange={(event) => setAccentKey(event.target.value)}>{accentOptions.map((accent) => <option key={accent} value={accent}>{accent[0]!.toUpperCase() + accent.slice(1)}</option>)}</Select></Field></div><Field label={child ? 'New PIN (optional)' : 'PIN'} hint={child ? 'Leave empty to keep the current PIN. Changing it locks active child sessions.' : 'Use 4–6 numbers.'}><TextInput type="password" inputMode="numeric" pattern="[0-9]{4,6}" value={pin} onChange={(event) => setPin(event.target.value)} required={!child} /></Field>{error ? <InlineNotice tone="error">{error}</InlineNotice> : null}<div className="modal-actions"><Button variant="secondary" onClick={onClose}>Cancel</Button><Button type="submit" disabled={busy}>{busy ? 'Saving…' : child ? 'Save profile' : 'Add child'}</Button></div></form>;
}

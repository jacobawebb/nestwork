...
import { ArrowDown, ArrowUp, Banknote, Flag, History, Plus, SlidersHorizontal, Target } from 'lucide-react';
import { useSearchParams } from 'react-router';
import type { ChildSummary, Goal, Household, LedgerEntry } from '@/app/types';
import { Avatar } from '@/components/avatar';
import { LedgerChoreModal } from '@/components/ledger-chore-modal';
...
export default function PiggyBanksPage() {
  ...
  const requestedId = search.get('child');
  const selectedId = dashboard.data?.children.find((child) => child.id === requestedId)?.id ?? dashboard.data?.children[0]?.id ?? null;
  const ledger = useApiResource<LedgerEntry[]>(selectedId ? `/parent/ledger?childId=${encodeURIComponent(selectedId)}` : null);
  const goalsEnabled = dashboard.data?.household.settings.savingsGoalsEnabled ?? false;
  const goals = useApiResource<Goal[]>(selectedId && goalsEnabled ? `/parent/goals/${selectedId}` : null);
  ...
  const closeLedger = () => { setLedgerMode(null); if (search.has('payout')) { const next = new URLSearchParams(search); next.delete('payout'); setSearch(next, { replace: true }); } };
  ...
  const moveGoal = async (goal: Goal, direction: -1 | 1) => {
    const active = (goals.data ?? []).filter((item) => item.active);
    const from = active.findIndex((item) => item.id === goal.id);
    const to = from + direction;
    if (from < 0 || to < 0 || to >= active.length || !child) return;
    const ordered = active.map((item) => item.id);
    [ordered[from], ordered[to]] = [ordered[to]!, ordered[from]!];
    try { await postJson('/parent/goals/reorder', { childId: child.id, goalIds: ordered }); await goals.reload(); }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Goal order could not be saved.'); }
  };
  ...
  return <div><header className="page-heading"><div><h1>Piggy banks</h1><p>Balances are calculated from an append-only ledger. The app records real-world money; it never transfers funds.</p></div>{child ? <div className="page-actions"><Button variant="secondary" onClick={() => setLedgerMode('ADJUSTMENT')}><SlidersHorizontal size={18} />Adjustment</Button><Button onClick={() => setLedgerMode('PAYOUT')}><Banknote size={18} />Record payout</Button></div> : null}</header>
    ...
    {dashboard.data?.children.length === 0 ? <EmptyState icon={<Banknote />} title="No piggy banks yet">Add a child profile first. A balance starts at zero and changes only through real ledger entries.</EmptyState> : <div className="bank-layout"><aside className="child-picker" aria-label="Choose child">{dashboard.data?.children.map((item) => <button key={item.id} type="button" aria-pressed={item.id === selectedId} onClick={() => { setSelectedChoreId(null); const next = new URLSearchParams(search); next.set('child', item.id); setSearch(next); }}><Avatar avatarKey={item.avatarKey} accentKey={item.accentKey} size="sm" /><span>{item.displayName}</span></button>)}</aside>{child && household ? <div className="bank-content">
      ...
      <div className="bank-columns"><section className="section-panel"><div className="section-heading"><div><h2>Savings goals</h2><p>Every goal uses the same available balance; money is not reserved.</p></div>{goalsEnabled ? <Button variant="secondary" size="sm" onClick={() => setGoalEditor('new')}><Plus size={16} />Goal</Button> : null}</div><div className="section-body goal-list">{!goalsEnabled ? <InlineNotice tone="info">Savings goals are disabled in household settings. Existing goals are preserved if the owner enables them again.</InlineNotice> : goals.loading && !goals.data ? <LoadingBlock /> : goals.data?.filter((goal) => goal.active).map((goal, index, activeGoals) => <article className="goal-row" key={goal.id}><div className="goal-icon"><Target /></div><div><div className="goal-title"><h3>{goal.name}</h3><span>{goal.progressPercent}%</span></div><div className="progress-track"><span style={{ width: `${goal.progressPercent}%` }} /></div><p><Money amountMinor={goal.progressMinor} currency={household.currency} locale={household.locale} /> of <Money amountMinor={goal.targetMinor} currency={household.currency} locale={household.locale} />{goal.spotlight ? ' · Spotlight' : ''}</p></div><div className="goal-actions"><Button variant="quiet" size="sm" aria-label={`Move ${goal.name} up`} disabled={index === 0} onClick={() => void moveGoal(goal, -1)}><ArrowUp size={16} /></Button><Button variant="quiet" size="sm" aria-label={`Move ${goal.name} down`} disabled={index === activeGoals.length - 1} onClick={() => void moveGoal(goal, 1)}><ArrowDown size={16} /></Button><Button variant="quiet" size="sm" onClick={() => setGoalEditor(goal)}>Edit</Button><Button variant="quiet" size="sm" onClick={() => void archiveGoal(goal)}>Archive</Button></div></article>)}{goalsEnabled && goals.data?.filter((goal) => goal.active).length === 0 ? <EmptyState icon={<Flag />} title="No active goals">Add a milestone without moving or reserving any money.</EmptyState> : null}</div></section>
        <section className="section-panel"><div className="section-heading"><div><h2>Ledger history</h2><p>Corrections are new entries; history stays intact.</p></div><Button variant="quiet" size="sm" onClick={() => setLedgerMode('REVERSAL')}><History size={16} />Reversal</Button></div><div className="ledger-list">{ledger.loading && !ledger.data ? <LoadingBlock /> : ledger.data?.map((entry) => <div className="ledger-row" key={entry.id}><span className={`ledger-type ledger-type-${entry.type.toLowerCase()}`}>{entry.type}</span><div><strong>{entry.reason}</strong>{entry.choreInstanceId && entry.choreTitle ? <button type="button" className="ledger-chore-link" onClick={() => setSelectedChoreId(entry.choreInstanceId)} aria-label={`View chore ${entry.choreTitle}`}>Task: {entry.choreTitle}</button> : null}<small>{new Intl.DateTimeFormat(household.locale, { dateStyle: 'medium', timeStyle: 'short', timeZone: household.timeZone }).format(new Date(entry.createdAt))}</small></div><strong className={entry.amountMinor >= 0 ? 'amount-positive' : 'amount-negative'}><Money sign amountMinor={entry.amountMinor} currency={entry.currency} locale={household.locale} /></strong></div>)}{ledger.data?.length === 0 ? <EmptyState icon={<History />} title="No ledger entries">Approved chores, payouts, and corrections will appear here.</EmptyState> : null}</div></section>
      </div>
    </div> : null}</div>}
    ...
    {child && goalsEnabled && goalEditor ? <Modal title={goalEditor === 'new' ? `Add a goal for ${child.displayName}` : `Edit ${goalEditor.name}`} onClose={() => setGoalEditor(null)}><GoalForm childId={child.id} goal={goalEditor === 'new' ? null : goalEditor} onClose={() => setGoalEditor(null)} onSaved={reloadDetails} /></Modal> : null}
    {selectedChoreId && household ? <LedgerChoreModal key={selectedChoreId} choreId={selectedChoreId} viewer="parent" locale={household.locale} onClose={() => setSelectedChoreId(null)} /> : null}
  </div>;
}

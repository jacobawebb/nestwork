...
import { Button, Field, InlineNotice, LoadingBlock, TextInput } from '@/components/ui';
...
export default function InvitePage() {
  ...
  const accept = async (event: FormEvent) => {
    ...
    try { await postJson(`/invitations/${encodeURIComponent(token)}/accept`, { displayName, password }); setComplete(true); }
  ...
  return <main className="setup-unlock"><section className="setup-unlock-card">
    {complete ? <><div className="complete-mark"><CheckCircle2 /></div><h1>You’re ready to join</h1><p>Your parent account has been created. Select your profile and sign in with the email and password you just chose.</p><Button onClick={() => window.location.assign('/')}>Choose profile</Button></> : <>
      ...
      {error ? <InlineNotice tone="error">{error}</InlineNotice> : data ? <form className="form-stack" onSubmit={accept}><p>This single-use invitation is for <strong>{data.email}</strong>.</p><Field label="Your display name"><TextInput value={displayName} onChange={(event) => setDisplayName(event.target.value)} required autoFocus /></Field><Field label="Create a password" hint="At least 12 characters with uppercase, lowercase, and a number."><TextInput type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} required /></Field>{formError ? <InlineNotice tone="error">{formError}</InlineNotice> : null}<Button type="submit" size="lg" disabled={busy}>{busy ? 'Creating account…' : 'Join household'}</Button></form> : null}
